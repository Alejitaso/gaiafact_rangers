    ...
    const handleProductSearch = async (searchType, searchValue) => {
        try {
            ...
            if (searchType === 'barcode') {
                // Buscar por código de barras (asumiendo que está en un campo del producto)
            ...
            if (producto) {
                // Cerramos el popup principal antes de mostrar el de SweetAlert
                cerrarPopup();
                mostrarConfirmacionProducto(producto);
            } else {
                // Cerramos el popup principal antes de mostrar el error
                cerrarPopup();
                mostrarError(
                    'Producto no encontrado',
                    `No se encontró ningún producto con ${searchType === 'barcode' ? 'código de barras' : 'ID'}: ${searchValue}`
                );
            }
        } catch (error) {
            setEsperandoProducto(false);
